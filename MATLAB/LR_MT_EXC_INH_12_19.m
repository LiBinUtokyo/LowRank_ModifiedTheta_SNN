...
% lowrank给EE，读EE
Sti_go = normrnd(0,2,1,N_E); %取自标准差为2的高斯分布
% Sti_go = 1./(1+exp(-Sti_go))*2; %施加Sigmoid并乘二
Sti_nogo = normrnd(0,2,1,N_E); %取自标准差为2的高斯分布
% Sti_nogo = 1./(1+exp(-Sti_nogo))*2; %施加Sigmoid并乘二

W_out = normrnd(0,2,N_E,1);
...
%     firings_exc_2 = [];
    ...
    firings_inh_1 = zeros(2000,2);
    LI1 = 0;
    firings_exc_1 = zeros(2000,2);
    LE1 = 0;
    firings_inh_2 = zeros(2000,2);
    LI2 = 0;
    firings_exc_2 = zeros(2000,2);
    LE2 = 0;
...
%     firings_exc_2 = [firings_exc_2;t+0*find(A(1:N_E,2)),find(A(1:N_E,2))];
    ...
    firings_inh_1(LI1+1:LI1+length(find(A(N_E+1:end,1))),:) = [t+0*find(A(N_E+1:end,1)),find(A(N_E+1:end,1))];
    firings_exc_1(LE1+1:LE1+length(find(A(1:N_E,1))),:) = [t+0*find(A(1:N_E,1)),find(A(1:N_E,1))];

    firings_inh_2(LI2+1:LI2+length(find(A(N_E+1:end,2))),:) = [t+0*find(A(N_E+1:end,2)),find(A(N_E+1:end,2))];
    firings_exc_2(LE2+1:LE2+length(find(A(1:N_E,2))),:) = [t+0*find(A(1:N_E,2)),find(A(1:N_E,2))];


    LI1 = LI1+length(find(A(N_E+1:end,1)));
    LE1 = LE1 + length(find(A(1:N_E,1)));

    LI2 = LI2+length(find(A(N_E+1:end,2)));
    LE2 = LE2 + length(find(A(1:N_E,2)));

end

firings_inh_1(all(firings_inh_1==0,2),:) = [];
firings_exc_1(all(firings_exc_1==0,2),:) = [];
firings_inh_2(all(firings_inh_2==0,2),:) = [];
firings_exc_2(all(firings_exc_2==0,2),:) = [];
