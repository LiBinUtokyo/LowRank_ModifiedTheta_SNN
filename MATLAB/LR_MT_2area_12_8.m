...
conn_EmEs = 2*randn(N_E/2,N_E/2); % 连接矩阵(Em←Es)
% conn_EmEs = 2*randn(N_E/2,1); % 连接矩阵(Em←Es),标准差为2的高斯分布
conn_EmEs = 1./(1+exp(-conn_EmEs))*2; %施加Sigmoid并乘二


conn_EsEm = 2*randn(N_E/2,N_E/2); % 连接矩阵(Es←Em)
% conn_EsEm = 2*randn(N_E/2,1); % 连接矩阵(Es←Em),标准差为2的高斯分布
conn_EsEm = 1./(1+exp(-conn_EsEm))*2; %施加Sigmoid并乘二


conn_EmEm = 2*randn(N_E/2,N_E/2); % 连接矩阵(Em←Em)
% conn_EmEm = 2*randn(N_E/2,1); % 连接矩阵(Em←Em),标准差为2的高斯分布
conn_EmEm = 1./(1+exp(-conn_EmEm))*2; %施加Sigmoid并乘二



conn_EsIs = 2*randn(N_E/2,N_I/2); % 连接矩阵(Es←Is)
% conn_EsIs = 2*randn(N_E/2,1); % 连接矩阵(E←I),标准差为2的高斯分布
conn_EsIs = 1./(1+exp(-conn_EsIs))*2; %施加Sigmoid并乘二

conn_EmIs = 2*randn(N_E/2,N_I/2); % 连接矩阵(Em←Is)
% conn_EmIs = 2*randn(N_E/2,1); % 连接矩阵(E←I),标准差为2的高斯分布
conn_EmIs = 1./(1+exp(-conn_EmIs))*2; %施加Sigmoid并乘二

conn_EsIm = 2*randn(N_E/2,N_I/2); % 连接矩阵(Es←Im)
% conn_EsIm = 2*randn(N_E/2,1); % 连接矩阵(E←I),标准差为2的高斯分布
conn_EsIm = 1./(1+exp(-conn_EsIm))*2; %施加Sigmoid并乘二

conn_EmIm = 2*randn(N_E/2,N_I/2); % 连接矩阵(Em←Im)
% conn_EmIm = 2*randn(N_E/2,1); % 连接矩阵(E←I),标准差为2的高斯分布
conn_EmIm = 1./(1+exp(-conn_EmIm))*2; %施加Sigmoid并乘二


conn_IsEs = 2*randn(N_I/2,N_E/2); % 连接矩阵(Is←Es)
% conn_IsEs = 2*randn(N_I/2,1); % 连接矩阵(Is←Es),标准差为2的高斯分布
conn_IsEs = 1./(1+exp(-conn_IsEs))*2; %施加Sigmoid并乘二

conn_ImEs = 2*randn(N_I/2,N_E/2); % 连接矩阵(Im←Es)
% conn_ImEs = 2*randn(N_I/2,1); % 连接矩阵(Im←Es),标准差为2的高斯分布
conn_ImEs = 1./(1+exp(-conn_ImEs))*2; %施加Sigmoid并乘二

conn_IsEm = 2*randn(N_I/2,N_E/2); % 连接矩阵(Is←Em)
% conn_IsEm = 2*randn(N_I/2,1); % 连接矩阵(Is←Em),标准差为2的高斯分布
conn_IsEm = 1./(1+exp(-conn_IsEm))*2; %施加Sigmoid并乘二

conn_ImEm = 2*randn(N_I/2,N_E/2); % 连接矩阵(Im←Em)
% conn_ImEm = 2*randn(N_I/2,1); % 连接矩阵(Is←Es),标准差为2的高斯分布
conn_ImEm = 1./(1+exp(-conn_ImEm))*2; %施加Sigmoid并乘二


conn_IsIs = 2*randn(N_I/2,N_I/2); % 连接矩阵(Is←Is)
% conn_IsIs = 2*randn(N_I/2,1); % 连接矩阵(Is←Is),标准差为2的高斯分布
conn_IsIs = 1./(1+exp(-conn_IsIs))*2; %施加Sigmoid并乘二

conn_ImIs = 2*randn(N_I/2,N_I/2); % 连接矩阵(Im←Is)
% conn_ImIs = 2*randn(N_I/2,1); % 连接矩阵(Im←Is),标准差为2的高斯分布
conn_ImIs = 1./(1+exp(-conn_ImIs))*2; %施加Sigmoid并乘二

conn_IsIm = 2*randn(N_I/2,N_I/2); % 连接矩阵(Is←Im)
% conn_IsIm = 2*randn(N_I/2,1); % 连接矩阵(Is←Im),标准差为2的高斯分布
conn_IsIm = 1./(1+exp(-conn_IsIm))*2; %施加Sigmoid并乘二

conn_ImIm = 2*randn(N_I/2,N_I/2); % 连接矩阵(Im←Im)
% conn_ImIm = 2*randn(N_I/2,1); % 连接矩阵(Im←Im),标准差为2的高斯分布
conn_ImIm = 1./(1+exp(-conn_ImIm))*2; %施加Sigmoid并乘二
...
% lowrank给EE，读EmEm
Sti_go = 2*rand(1,N_E/2); %取自标准差为2的高斯分布
Sti_go = 1./(1+exp(-Sti_go))*2; %施加Sigmoid并乘二
Sti_nogo = 2*rand(1,N_E/2); %取自标准差为2的高斯分布
Sti_nogo = 1./(1+exp(-Sti_nogo))*2; %施加Sigmoid并乘二

W_out = conn_EsEs;
% W_out = conn_IsIs;
% W_out = 2*randn(N_E/2,1);

[conn_EsEs, conn_EmEs, conn_EsEm, conn_EmEm] = deal(RS*randn(N_E/2,N_E/2)*(1/N_E) + W_out*Sti_go); %连接矩阵(E←E),随机加lowRank, con_EE = RS*X+P, P=W_out*Sti_go
...
ylim([min(min(out))*1.1 max(max(out))*1.1+0.001])
