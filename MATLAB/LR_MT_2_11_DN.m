% 分成E-I两组的QIF modified theta model
%Detection of a Noisy Stimulus
%只考虑兴奋性连接，因为有没有其他连接真的没有任何影响，或许可以和前辈讨论一下
%将连接概率尽可能增大
% 连接是一对一生成的（非平均场）
% Go_noGo Task
% 同时观察两个输入
%用两个垂直的01向量构筑lowrank connectivity
%理解：如果把go输入看成一个pattern，那么越接近go输入，就越能引起神经网络反应，就像hopfield联想记忆模型一样
%这个时候即使全部为正，也可以定义垂直，即在go的pattern的位置上全是零就是完全与go输入一点也不相关
%把连接矩阵看成是连接的概率，在这里本来值就相当小了
%笔记：
%高斯分布随机数 r = normrnd(mu,sigma,sz)
%gamma分布（指数分布随机数）（gamma（1,λ）=exp(λ)）r = exprnd(mu,sz1,...,szN) mu为均值
%真正的gamma分布 r = gamrnd(a,b,sz1,...,szN)
%gamma分布均值为a*b,方差为a*b^2
%对数正态分布 r = lognrnd(mu,sigma,sz1,...,szN)
tic
clear

%% Setting

c_values = [0.2 0.4 0.6 0.9];

N_E = 1000; % 神经元数(E)
N_I = 0; % 神经元数(I)
N = N_E+N_I; % 神经元数(Total)
dt = 0.01; % 运算步长(ms/step)
T = 30; % 模拟时间总长度(ms)
tt = T/dt; % 计算步数
theta = ones(N,length(c_values))*V2theta(-70); % 位相初始值
gLE = 0.08; % 漏电导（E）
gLI = 0.1; % 漏电导（I）
g_p = [0.004069, 0.02672, 0.003276, 0.02138]; %突触电导的权重(E←E, E←I, I←E, I←I)
V_T = -55; % 点火阈值(mV)
V_R = -62; % 静息电位
V_E = 0; % 兴奋性突触反转电位
V_I = -70; % 抑制性突触反转电位

out = zeros(tt,length(c_values));

RS = 1; %RandomStrength
IS = 3;% Input Strength,if it is 1, n=IS*Sti_go

h = 2/(V_T-V_R);
q = [(2*V_E-V_R-V_T)/(V_T-V_R), (2*V_I-V_R-V_T)/(V_T-V_R)];
tau_dE = 2; % 衰减时间常数（E）(ms)
tau_dI = 5; % 衰减时间常数（I）(ms)
%n,m的参数
mu_n = 50;
si_n = 10;

b_n = si_n^2/mu_n;%gamma分布
a_n = mu_n/b_n;

% normrnd(mu_n,si_n)
% gamrnd(a_n,b_n)

mu_m = 50;
si_m = 10;

b_m = si_m^2/mu_m;%gamma分布
a_m = mu_m/b_m;

rhon = 2;%Connectivity overlap for n
rhom = 2;%Connectivity overlap for m

% conn_EI = RS*gamrnd(a, b, N_E,N_I)*(1/N_I); % 连接矩阵(E←I)
conn_EI = RS*rand([N_E,N_I])*(1/N_I); % 连接矩阵(E←I)
% conn_IE = RS*gamrnd(a, b, N_I,N_E)*(1/N_E); % 连接矩阵(I←E)
conn_IE = RS*rand([N_I,N_E])*(1/N_E); % 连接矩阵(I←E)
% conn_II = RS*gamrnd(a, b, N_I,N_I)*(1/N_I); % 连接矩阵(I←I)
conn_II = RS*rand([N_I,N_I])*(1/N_I); % 连接矩阵(I←I)


% lowrank给EE，读EE
%Sti_go,nogo,W_out三个向量要保证互相垂直同时还非负，那么就只能让有值的位置互相不重合
%随机创建一个N维向量，对其中小于1/3的位置给sti_go，即不小于1/3的全部变成零，其余同理
%改进点：其中的值要改成高斯分布的，好像也没有必要，只要不在一个位置上有重叠，任何值都无所谓
%改进点：原文献中sti的数值取自均值为零，标准差为2的高斯分布
%改进点：P的量级调整应该更加自然一点
%改进：完全将位置的选择变为随机，以便观察情况？

Nic = 12;%Number of realization of the random input
sigma = 0.4;%Std of noise in input


c1=0.9;
c2=0.1;
temp = rand([1,N_E]);

Sti = temp;
Sti(Sti>=1/30) = 0;
% Sti(Sti~=0) = normrnd(mu_n,si_n,1,length(find(Sti~=0)));
Sti(Sti~=0) = gamrnd(a_n,b_n,1,length(find(Sti~=0)));
Sti(Sti<0) = 0;

W_out = temp;
W_out(W_out<29/30) = 0;
% W_out(W_out~=0) = normrnd(mu_m,si_m,1,length(find(W_out~=0)));
W_out(W_out~=0) = gamrnd(a_m,b_m,1,length(find(W_out~=0)));
W_out = W_out';
W_out(W_out<0) = 0;

y = abs(normrnd(0,1,N_E,1)); %Direcrion of connectivity overlap
%h %Orthogonal noise in input

W = RS*normrnd(0,sqrt(1/N_E),N_E,N_E);
W = abs(W);

m = W_out + rhom*y;
n = Sti + rhon*y';
P = (m*n)/N_E;

disp(['Random Strength: ' num2str(RS)])
disp(['Connectivity Strength: ' num2str(n*m/N_E)])

conn = W + P; %连接矩阵(N*N),随机加lowRank, con = RS*X+P, P=W_out*Sti/N
conn_EE = conn;

rec_V = zeros(N,tt,length(c_values)); % 记录膜电位
A = zeros(N,1,length(c_values)); % 点火的记录

g_EE = zeros(N_E,tt,length(c_values)); % 突触电导(E←I)
g_EI = zeros(N_E,tt,length(c_values)); % 突触电导(E←I)
g_IE = zeros(N_I,tt,length(c_values)); % 突触电导(I←E)
g_II = zeros(N_I,tt,length(c_values)); % 突触电导(I←I)

% g_EE(:,:,1) = abs(normrnd(0,1,N_E,tt)); %g的初始值

% 点火的记录(最后画图用)

firings_exc = zeros(N_E,tt,length(c_values));
firings_inh = zeros(N_I,tt,length(c_values));

%     firings_inh_1 = [];
%     firings_exc_1 = [];
%
%     firings_inh_2 = [];
%     firings_exc_2 = [];


%% Main
for t = 2:tt
    I_E = 0;
    I_I = 0;
    %     if 5 < t/100 &&  t/100 <15
    if 5 < t/100
        I_E = Sti(1:N_E)'; % 输入电流(E)
    end

    if mod(t,tt/100)==0
        disp([num2str(t/100),'ms'])
    end

    for i = 1:length(c_values)
        % 计算突触电导
        g_EE(:,t,i) = g_EE(:,t-1,i) + (-g_EE(:,t-1,i)/tau_dE+g_p(1)*conn_EE*A(1:N_E,i))*dt;
        g_EI(:,t,i) = g_EI(:,t-1,i) + (-g_EI(:,t-1,i)/tau_dI+g_p(2)*conn_EI*A(N_E+1:N,i))*dt;
        g_IE(:,t,i) = g_IE(:,t-1,i) + (-g_IE(:,t-1,i)/tau_dE+g_p(3)*conn_IE*A(1:N_E,i))*dt;
        g_II(:,t,i) = g_II(:,t-1,i) + (-g_II(:,t-1,i)/tau_dI+g_p(4)*conn_II*A(N_E+1:N,i))*dt;

        % 计算位相(膜电位)
        pre_theta = theta;
        % E
        tmp_theta = pre_theta(1:N_E,i);
        theta(1:N_E,i) = tmp_theta + (-gLE*cos(tmp_theta)+h*(1+cos(tmp_theta)).*I_E*c_values(i)+g_EE(:,t,i).*(q(1)*(1+cos(tmp_theta))- ...
            sin(tmp_theta))+g_EI(:,t,i).*(q(2)*(1+cos(tmp_theta))-sin(tmp_theta)))*dt + sqrt(dt)*sigma*I_E*normrnd(0,1,1);
        % I
        tmp_theta = pre_theta(N_E+1:N,i);
        theta(N_E+1:N,i) = tmp_theta + (-gLI*cos(tmp_theta)+h*(1+cos(tmp_theta)).*I_I*c_values(i)+g_IE(:,t,i).*(q(1)*(1+cos(tmp_theta))- ...
            sin(tmp_theta))+g_II(:,t,i).*(q(2)*(1+cos(tmp_theta))-sin(tmp_theta)))*dt+ sqrt(dt)*sigma*I_I*normrnd(0,1,1);
    end
    % 保存膜电位
    rec_V(:,t,:) = (V_T+V_R)/2+(V_T-V_R)/2*tan(theta/2);

    % 处理点火了的神经元
    A = (theta >= pi);
    theta(theta >= pi) = theta(theta >= pi)-2*pi;
    % 保存点火情报

    firings_exc(:,t,:) = A(1:N_E,:);
    firings_inh(:,t,:) = A(N_E+1:end,:);

    %     firings_inh_1 = [firings_inh_1;t+0*find(A(N_E+1:end,1)),find(A(N_E+1:end,1))];
    %     firings_exc_1 = [firings_exc_1;t+0*find(A(1:N_E,1)),find(A(1:N_E,1))];
    %
    %     firings_inh_2 = [firings_inh_2;t+0*find(A(N_E+1:end,2)),find(A(N_E+1:end,2))];
    %     firings_exc_2 = [firings_exc_2;t+0*find(A(1:N_E,2)),find(A(1:N_E,2))];

end
g = [g_EE + g_EI ; g_IE + g_II];
%输出 读EE
% out = [tanh(g_EE(:,:,1)')*W_out/N_E tanh(g_EE(:,:,2)')*W_out/N_E];
for i = 1:length(c_values)
out(:,i) = tanh(g_EE(:,:,i)')*W_out/N_E;
end
%% Figure
xaxis = dt:dt:T;

% 连接矩阵描画,因为抑制性神经元少，所以公式得到的抑制性连接强
figure
imagesc([[conn_EE,conn_EI];[conn_IE,conn_II]])
xlabel('From')
ylabel('To')
colormap('gray');
colorbar
% clim([0 0.001]);
%pbaspect([1,1,1])
title('Connectivity')

disp(['Random Strength: ' num2str(RS)])
disp(['Connectivity Strength: ' num2str(n*m/N_E)])

for i = 1 : length(c_values)
[x,y] = find(firings_exc(:,:,i));
figure
% 発火情報描画

subplot(3,1,1)
scatter(y*dt,x,2,'red','filled')
xlim([0 T])
title(['spikes raster for c = ' num2str(c_values(i))])

% 膜電位描画(一部分)
subplot(3,1,2)
plot(xaxis,rec_V(1:floor(N_E*0.1)+1,:,i),'r')
hold on
% plot(xaxis,rec_V(N_E+1:floor(N_E+N_I*0.1)+1,:,o),'b')
hold off
xlim([0 T])
ylim([-200 100])
title(['voltage for c = ' num2str(c_values(i))])



% 突触电导描画
subplot(3,1,3)
plot(xaxis, mean(g_EE(:,:,i)))
hold on
plot(xaxis, mean(g_EI(:,:,i)))
plot(xaxis, mean(g_IE(:,:,i)))
plot(xaxis, mean(g_II(:,:,i)))
plot(xaxis, mean(g(:,:,i)))
hold off
xlim([0 T])
title(['synaptic conductance for c = ' num2str(c_values(i))])
legend('g_{EE}','g_{EI}','g_{IE}','g_{II}', 'g', 'Location','West')

% figure
% for i = 1:5
% plot(xaxis, g_EE(i,:,o))
% hold on
% end
end

% %观察输出
figure
limy = 3;
x = [5 5 30 30];
y = [-limy limy limy -limy];
patch(x,y,'blue','FaceAlpha',0.2,'EdgeColor','none','DisplayName','Stimulate time')
hold on
for i = 1:length(c_values)
plot(xaxis, out(:,i),'LineWidth',2,'DisplayName',['c = ' num2str(c_values(i))]);
end
title('Readout from Excitatory neurons')
ylim([min(min(out))*1.1 max(max(out))*1.1])
legend()
% legend('O_{go}', 'O_{nogo}')

% op(1).LineWidth = 2;
% op(1).Color = [0.24 0.35 0.67]; %钴色
% op(2).LineWidth = 2;
% op(2).Color = [0.01 0.66 0.62];% 锰蓝

%观察平均活动
figure
for i = 1:length(c_values)

on_m = W_out'*g_EE(:,:,i);
on_I = Sti*g_EE(:,:,i);
plot(on_I,on_m,'LineWidth',2,'DisplayName',['c = ' num2str(c_values(i))]);
hold on

% Activity(1).Color = [0.24 0.35 0.67];%钴色
% yl = ylim;
% xl = xlim;

% hold on
% figure
% Activity(2) = plot(on_Inogo,on_m_nogo);
% xlabel('Sti_{nogo}')
% ylabel('m')
% Activity(2).LineWidth = 2;
% Activity(2).Color = [0.01 0.66 0.62];% 锰蓝
% ylim(yl)
% xlim(xl)
end
xlabel('I')
ylabel('m')
legend()

toc
%%
% W2 = gamrnd(a,b,N_E,1)/N_E;
% out2 = [tanh(g_EE(:,:,1)')*W2 tanh(g_EE(:,:,2)')*W2];
% figure
% plot(xaxis, out2(:,1))
% hold on
% plot(xaxis, out2(:,2))

% for o=1
% figure
% for i = 5:15
% plot(xaxis,g_EE(i,:,o))
% hold on
% end
% title("g_{rec}"+num2str(o))
% % xlim([20 30])
% end

%% function

% function V=theta2V(theta)
%     V_T = -55; % 点火阈值
%     V_R = -62; % 静息电位
%     V = (V_T+V_R)/2+(V_T-V_R)/2*tan(theta/2);
% end

function theta = V2theta(V)
    V_T = -55; % 点火阈值
    V_R = -62; % 静息电位
    theta = 2*atan((V-(V_R+V_T)/2)*2/(V_T-V_R));
end