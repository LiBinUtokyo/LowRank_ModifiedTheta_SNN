...
% lowrank给EE，读EE
Sti_go = 2*rand(1,N); %取自标准差为2的高斯分布
% Sti_go = 1./(1+exp(-Sti_go))*2; %施加Sigmoid并乘二
Sti_nogo = 2*rand(1,N); %取自标准差为2的高斯分布
% Sti_nogo = 1./(1+exp(-Sti_nogo))*2; %施加Sigmoid并乘二

W_out = 2*rand(N,1);

conn = RS*randn(N,N)*(1/N) + W_out*Sti_go; %连接矩阵(N*N),随机加lowRank, con = RS*X+P, P=W_out*Sti_go
conn_EE = conn(1:N_E,1:N_E);
conn_IE = conn(N_E+1:end,1:N_E);
conn_EI = conn(1:N_E,N_E+1:end);
conn_II = conn(N_E+1:end,N_E+1:end);
...
I_I = [Sti_go(N_E+1:end)' Sti_nogo(N_E+1:end)']; % 输入电流(I)
...
%输出 读EE
out = [tanh(g(:,:,1)')*W_out/N tanh(g(:,:,2)')*W_out/N];
