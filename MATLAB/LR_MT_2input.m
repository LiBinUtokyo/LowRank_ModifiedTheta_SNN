...
conn_IE = 1./(1+exp(-conn_IE))*4; %施加Sigmoid并乘二

conn_II = 2*randn(N_I,N_I); % 连接矩阵(I←I)
% conn_II = 2*randn(N_I,1); % 连接矩阵(I←I),标准差为2的高斯分布
conn_II = 1./(1+exp(-conn_II))*4; %施加Sigmoid并乘二
...
%输出 读IE
% out = [tanh(g_IE(:,:,1)')*W_out/N_I tanh(g_IE(:,:,2)')*W_out/N_I];

%输出 读EI
% out = [tanh(g_EI(:,:,1)')*W_out/N_E tanh(g_EI(:,:,2)')*W_out/N_E];

%输出 读II
% out = [tanh(g_II(:,:,1)')*W_out/N_I tanh(g_II(:,:,2)')*W_out/N_I];
