...
% y = abs(normrnd(0,1)); %Direcrion of connectivity overlap 
...
%     g_EE(g_EE<0) = 0;
    ...
    theta(1:N_E,1) = tmp_theta + (-gLE*cos(tmp_theta)+h*(1+cos(tmp_theta)).*I_E(:,1)*c1+g_EE(:,t,1).*(q(1)*(1+cos(tmp_theta))- ...
        sin(tmp_theta))+g_EI(:,t,1).*(q(2)*(1+cos(tmp_theta))-sin(tmp_theta)))*dt + sqrt(dt)*sigma*I_E(:,1)*normrnd(0,1,1);
    % I
    tmp_theta = pre_theta(N_E+1:N,1);
    theta(N_E+1:N,1) = tmp_theta + (-gLI*cos(tmp_theta)+h*(1+cos(tmp_theta)).*I_I(:,1)*c1+g_IE(:,t,1).*(q(1)*(1+cos(tmp_theta))- ...
        sin(tmp_theta))+g_II(:,t,1).*(q(2)*(1+cos(tmp_theta))-sin(tmp_theta)))*dt+ sqrt(dt)*sigma*I_I(:,1)*normrnd(0,1,1);

    % E
    tmp_theta = pre_theta(1:N_E,2);
    theta(1:N_E,2) = tmp_theta + (-gLE*cos(tmp_theta)+h*(1+cos(tmp_theta)).*I_E(:,1)*c2+g_EE(:,t,2).*(q(1)*(1+cos(tmp_theta))- ...
        sin(tmp_theta))+g_EI(:,t,2).*(q(2)*(1+cos(tmp_theta))-sin(tmp_theta)))*dt+ sqrt(dt)*sigma*I_E(:,1)*normrnd(0,1,1);
    % I
    tmp_theta = pre_theta(N_E+1:N,2);
    theta(N_E+1:N,2) = tmp_theta + (-gLI*cos(tmp_theta)+h*(1+cos(tmp_theta)).*I_I(:,1)*c2+g_IE(:,t,2).*(q(1)*(1+cos(tmp_theta))- ...
        sin(tmp_theta))+g_II(:,t,2).*(q(2)*(1+cos(tmp_theta))-sin(tmp_theta)))*dt+ sqrt(dt)*sigma*I_I(:,1)*normrnd(0,1,1);
