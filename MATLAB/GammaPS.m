...
function GammaPS(a,b)
%% 说明
%用于Parameter Search的函数
%gamma随机数参数,平均值为a*b,方差为a*b^2
%Shape parameter alpha = a
%Scale parameter b
...
conn_EI = RS*gamrnd(a, b, N_E,N_I)*(1/N_E); % 连接矩阵(E←I)
conn_IE = RS*gamrnd(a, b, N_I,N_E)*(1/N_I); % 连接矩阵(I←E)
conn_II = RS*gamrnd(a, b, N_I,N_I)*(1/N_I); % 连接矩阵(I←I)

% lowrank给EE，读EE
Sti_go = gamrnd(a, b, 1,N_E); %取自gamma分布
Sti_nogo = gamrnd(a, b, 1,N_E); %取自gamma分布

W_out = gamrnd(a, b, N_E,1);
...
xlim([0 T])
title("spikes " + num2str(o-1))

% 膜電位描画(一部分)
subplot(3,1,2)
plot(xaxis,rec_V(1:floor(N_E*0.1)+1,:,o-1),'r')
hold on
hold off
xlim([0 T])
ylim([-200 100])
...
% 突触电导描画
subplot(3,1,3)
plot(xaxis, mean(g_EE(:,:,o-1)))
hold on
plot(xaxis, mean(g_EI(:,:,o-1)))
plot(xaxis, mean(g_IE(:,:,o-1)))
plot(xaxis, mean(g_II(:,:,o-1)))
plot(xaxis, mean(g(:,:,o-1)))
hold off
xlim([0 T])
title("g " + num2str(o-1))
...
% mkdir output
% 
% print(connectivity,['Connectivity\Connect_',num2str(a),'_',num2str(b),'_',num2str(gap),'.png'],'-dpng')
% print(fvg(1),['fvg_go\fvg_',num2str(a),'_',num2str(b),'_',num2str(gap),'.png'],'-dpng')
% print(fvg(2),['fvg_nogo\fvg_',num2str(a),'_',num2str(b),'_',num2str(gap),'.png'],'-dpng')
% print(output,['output\ouput_',num2str(a),'_',num2str(b),'_',num2str(gap),'.png'],'-dpng')
% 
toc
% close all
%% function
